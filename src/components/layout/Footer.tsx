    ...
    <footer className="border-t border-border bg-footer-bg text-foreground">
      <div className="container-wide px-6 lg:px-8">
        <div className="flex flex-col items-start justify-between gap-8 py-16 sm:flex-row sm:items-center">
          <Link href="/" className="inline-flex items-center gap-5" aria-label={siteConfig.name}>
            <LogoIcon size={72} />
        ...
        <div className="grid gap-16 border-t border-border py-12 md:grid-cols-2 lg:grid-cols-4">
          <div>
            ...
            <ul className="mt-5 space-y-3 text-base leading-relaxed">
              ...
              <li className="pt-1 text-muted">{siteConfig.secretaryHours.weekdays}</li>
              <li className="text-muted">{siteConfig.secretaryHours.saturday}</li>
            </ul>
          </div>

          <div>
            <h2 className="font-display normal-case text-xl text-foreground">Links</h2>
            <ul className="mt-5 space-y-3 text-base lg:text-lg">
              {footerNavItems.map((item) => (
                <li key={item.href}>
                  <Link href={item.href} className="text-accent transition-colors hover:text-accent-light">
                    {item.label}
                  </Link>
                </li>
              ))}
          ...
          <div>
            <h2 className="font-display normal-case text-xl text-foreground">Horários rápidos</h2>
            <ul className="mt-5 space-y-3 text-base leading-relaxed">
              <li>
                <span className="font-medium text-foreground">{quickScheduleHighlight.label}</span>
                <br />
                <span className="text-foreground/80">{quickScheduleHighlight.times}</span>
              </li>
              <li className="text-foreground/80">{quickScheduleHighlight.location}</li>
              <li>
                <Link href="/horarios" className="text-accent transition-colors hover:text-accent-light">
