...
4. Capture method, in order:
   ...
   - `ScrollReveal` starts at `opacity-0`. A full-page shot leaves later sections blank unless you emulate reduced motion (Playwright `reducedMotion: 'reduce'`) or scroll them into view first.
...
5. **Read the images.** Check overflow, clipped text, contrast, unequal columns, missing times, controls that should be gone, mobile stacking. Overlay/lightbox controls: confirm they sit where intended (center of the sides vs stuck in a corner). Lightbox arrows must stay on the **viewport** edges after the next photo — not hug the first image’s width. Closed `<dialog>` must stay `display: none` (`hidden open:flex`, never a bare `flex`) or later carousels cover Fechar and steal taps. User-supplied cutouts (transparent PNG, or a JPEG whose black/white field is matte, not art): sit them on the page background — do not invent a matching color band. If you find a real UI bug, fix it, re-shot, re-read. Do not hand off a broken layout.
